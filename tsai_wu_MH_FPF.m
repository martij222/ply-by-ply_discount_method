function [ tsai_wu_MH_SR ] = tsai_wu_MH_FPF( local_stress, sig1_T_ult, sig1_C_ult, sig2_T_ult, sig2_C_ult, tau12_ult )
%tsai_wu_MH_FPF Compute strength ratio using Tsai-Wu failure theory (Mises-Hencky criterion)
...
%sr_H = roots( [a_H b c] ); sr_H = sr_H(sr_H > 0); % positive roots only
sr_MH = roots( [a_MH b c] ); sr_MH = sr_MH(sr_MH > 0); % positive roots only
